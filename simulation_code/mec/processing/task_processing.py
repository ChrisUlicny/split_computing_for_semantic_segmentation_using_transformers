...
from simulation_code.mec.entities import UE, BaseStation, MecEntity
from simulation_code.mec.mec_types import EntityID
from simulation_code.mec.tasks.task import Computing, Message, Task, TaskComputationEntity, ComputingUE
...
def process_ue_tasks_computation_round_robin(
    ...
    while ue.used_timestep_interval < sim_dt and not len(ue.tasks_computing) == 0:
        if should_wait(tasks, ue.used_timestep_interval):
            ...
            # if "waiting_time" not in task.task.data.keys():
            #     task.task.data["waiting_time"] = {}
            #     task.task.data["waiting_time"]["vehicle"] = 0
            # task.task.data["waiting_time"]["vehicle"] += waiting_timestamp
...
def process_tasks_computation_fifo(
    ...
    for task in tasks:
        ...
        # if "computation_time" not in task.task.data.keys():
        #     task.task.data["computation_time"] = {}
        #     task.task.data["computation_time"]["vehicle"] = 0
        #     task.task.data["computation_time"]["server"] = 0
        # task.task.data["computation_time"]["vehicle" if is_vehicle else "server"] += result.time_spent

        if "latencies" not in task.task.data.keys():
            task.task.data["latencies"] = {}

            task.task.data["latencies"]["processing"] = {}

            task.task.data["latencies"]["waiting"] = {}

            task.task.data["latencies"]["processing"]["vehicle"] = 0

            task.task.data["latencies"]["processing"]["server"] = 0

            task.task.data["latencies"]["waiting"]["vehicle"] = 0

            task.task.data["latencies"]["waiting"]["server"] = 0

        if "uplink" not in task.task.data.keys():
            task.task.data["uplink"] = 0


        task.task.data["latencies"]["processing"]["vehicle" if is_vehicle else "server"] += result.time_spent

        # waiting time - time the tasks lived minus time tasks spent processing and waiting (in other queues)
        task.task.data["latencies"]["waiting"]["vehicle" if is_vehicle else "server"] = (task.task.lived_timestep -
            task.task.data["latencies"]["waiting"]["server" if is_vehicle else "vehicle"] -
            task.task.data["latencies"]["processing"]["vehicle"] -
            task.task.data["latencies"]["processing"]["server"] - task.task.data["uplink"])

        print("Waiting server:", task.task.data["latencies"]["waiting"]["server"])
        print("Waiting vehicle:", task.task.data["latencies"]["waiting"]["vehicle"])
        print("Processing server:", task.task.data["latencies"]["processing"]["server"])
        print("Processing vehicle:", task.task.data["latencies"]["processing"]["vehicle"])
        print("Data transfer:", task.task.data["uplink"])
        print("Total task latency:", task.task.lived_timestep)
        print("Summed latencies:", task.task.data["latencies"]["waiting"]["vehicle"] + task.task.data["latencies"]["processing"]["vehicle"]
              + task.task.data["latencies"]["waiting"]["server"] + task.task.data["latencies"]["processing"]["server"] + task.task.data["uplink"])
...
def process_bs_tasks_computation_round_robin_per_task_owner(
    ...
    while dt_used < sim_dt:
        ...
        if not tasks:
            ...
            if "waiting_time" not in task.task.data.keys():
                task.task.data["waiting_time"] = {}
                task.task.data["waiting_time"]["server"] = 0
            task.task.data["waiting_time"]["server"] += new_dt
            dt_used = new_dt
            tasks = get_next_task_queue(ueid_tasks_mapping, per_owner_dt_used, dt_used, quantum, sim_dt)
            if not tasks:
                break
