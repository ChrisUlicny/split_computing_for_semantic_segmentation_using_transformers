...
from simulation_code.simulations.sc_ee_simulation.task.image_id_generator import RandomIdGenerator
...
from simulation_code.simulations.sc_ee_simulation.task.task_events import TaskEventsProtocol

from simulation_code.simulations.sc_ee_simulation.task.task_size_generator import OursTaskSizeGenerator
from simulation_code.simulations.sc_ee_simulation.simulation.main_simulation import MainSimulation

from simulation_code.simulations.sc_ee_simulation.common.stats_collector import DDPGStatsCollector
...
def execute_whole_simulation(config: SimulationConfig, config_data = None, idx = None, ep_idx=None):
    ...
    # responsible for adding vehicles to the simulation environment within the context of the IISMotion class
    vehicles = _add_vehicles(
        iismotion, config.vehicles, config.simulation.dt, 
        task_event_listener, SimulationType(config.simulation.simulation_type).name,
        sim_idx=idx, ep_idx=ep_idx
    )
    # contributes to setting up the base stations within the simulation environment, allowing for their interaction and influence on the simulated scenario
    base_stations = _add_base_stations(iismotion, config.base_stations)

    map_grid = iismotion.map.mapGrid
    # Represents a SINR (Signal-to-Interference-plus-Noise Ratio) map used for managing connections between user equipment (UE)
    # and base stations (BS) based on signal quality.
    sinr_map = SINRMap(
        x_size=40,
        y_size=40,
    ...
    #  represents a simulation environment for MEC systems, with capabilities to simulate network behavior,
    #  communication processes, and task computation, crucial for analyzing and optimizing MEC deployments
    mec = MECSimulation(
        ue_dict=vehicles,
    ...
    # creating stats collector - remove ddpg stats ?
    ddpg_stats = DDPGStatsCollector()

    # actually performs simulation steps and so on
